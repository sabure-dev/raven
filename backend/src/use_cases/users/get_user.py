...
class GetUserUseCase(BaseUseCase[GetUserInput, UserOut]):
    ...
    async def execute(self, input_data: GetUserInput) -> UserOut:
        user = await self.user_service.get_user_by_id(input_data.user_id, load_orders=True, load_bets=True)
        return user.to_read_model(include_orders=True, include_bets=True)
